...
public interface HistoryFlowMapper {
	...
	//////////////////////////////////////
	// @formatter:off
//	@Select("select * from (SELECT * FROM history_flow" 
...
//	public List<HistoryFlowEntity> selectAll(int from, int fetchCount);
	...
	// @formatter:off
	@Select("SELECT IfNull(max(cast(history_id as signed)),0) " 
			+ " FROM history_flow")
	// @formatter:on
	...
	// @formatter:off
	@Update(" insert into history_node( " 
			+ " 		flow_id, " 
			+ " 		node_id, " 
			+ " 		history_id, "
			+ " 		node_name, " 
	...
	// @formatter:off
	@Update(" insert into history_node_http( " 
			+ " 		flow_id, " 
			+ " 		node_id, " 
			+ " 		history_id, "
			+ " 		timeout, " 
	...
	// @formatter:off
	@Update(" insert into history_node_shell( " 
			+ " 		flow_id, " 
			+ " 		node_id, " 
			+ " 		history_id, "
			+ " 		timeout, " 
			...
			+ " 		now()," 
			+ " 		now()" 
			+ " 		from publish_node_shell where flow_id = #{param1} ")
	// @formatter:on
	public void createHistoryNodeShell(String flowId, String historyId, String userId);

	// @formatter:off
	@Update(" insert into history_node_roles( " 
			+ " 		flow_id, " 
			+ " 		node_id, " 
			+ " 		history_id, "
			+ " 		roles_id, " 
	...
	@Update("insert into history_edge( " 
			...
			+ " 	now()," 
			+ " 	now()" 
			+ "		from publish_edge where flow_id = #{param1} ")
	// @formatter:on
